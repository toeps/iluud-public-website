...
export class AddLocationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cityAddress = localStorage.getItem('CityAddress')
    this.setValue();
  }
    ...
    if(this.addressForm.valid){
      ...
      this.service.post1(data , "add_update_address").pipe().subscribe(response => {
        if(response.api_status){
          ...
          localStorage.setItem('suburb' , data.suburb )
          localStorage.setItem('city' , data.city )
          this.toastr.success(response.message);
          this.router.navigate(['/service/address-list'])
        }else{
          this.ngxService.stop();
  ...
  setValue() {
    ...
    this.officeForm.patchValue({
      officeCity: this.cityAddress,
    });
    this.otherForm.patchValue({
      otherCity: this.cityAddress,
    });
  }
}
